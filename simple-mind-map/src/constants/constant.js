...
export const cssContent = `
  ...
  /* 今日到期复习节点发光 */
  .smm-node-review-glow {
    animation: smm-node-glow var(--smm-glow-speed, 2s) ease-in-out infinite;
  }
  .smm-node-review-glow .smm-hover-node {
    display: block;
    opacity: 1;
    stroke-width: 2;
    stroke: var(--smm-review-glow-color, #409eff);
  }
  @keyframes smm-node-glow {
    0%, 100% {
      filter: drop-shadow(0 0 4px rgba(var(--smm-review-glow-rgb, 64, 158, 255), 0.7));
    }
    50% {
      filter: drop-shadow(0 0 var(--smm-glow-intensity, 12px) rgba(var(--smm-review-glow-rgb, 64, 158, 255), 0.95));
    }
  }

  /* 重点节点金色光晕 */
  .smm-node-focus-glow {
    animation: smm-node-focus-glow var(--smm-glow-speed, 2s) ease-in-out infinite;
  }
  .smm-node-focus-glow .smm-hover-node {
    display: block;
    opacity: 1;
    stroke-width: 2.5;
    stroke: var(--smm-focus-glow-color, #f59e0b);
  }
  @keyframes smm-node-focus-glow {
    0%, 100% {
      filter: drop-shadow(0 0 5px rgba(var(--smm-focus-glow-rgb, 245, 158, 11), 0.75));
    }
    50% {
      filter: drop-shadow(0 0 var(--smm-focus-glow-intensity, 15px) rgba(var(--smm-focus-glow-rgb, 245, 158, 11), 1));
    }
  }
