<template>
  ...
  >
    <template v-if="type === 'node'">
      ...
      <div
        class="item focus"
        @click="toggleFocusNode"
        :class="{ disabled: isGeneralization }"
      >
        <span class="name">{{ focusStatus ? '⭐️ 取消重点' : '⭐️ 标记重点' }}</span>
        <span class="desc">{{ focusStatus ? '√' : '' }}</span>
      </div>
      <div
        class="item"
      ...
      <div
        class="item"
        @click="openCardManager"
        :class="{ disabled: isGeneralization }"
      >
        <span class="name">{{ $t('contextmenu.manageCards') }}</span>
      </div>
